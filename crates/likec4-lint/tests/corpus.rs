...
use likec4_lint::{lint, LintConfig, Severity, SourceFile};
...
fn collect(dir: &Path, files: &mut Vec<PathBuf>, roots: &mut Vec<PathBuf>) {
    ...
    for path in entries {
        ...
        } else if likec4_lint::is_likec4_document(&path) {
            files.push(path);
        } else if likec4_lint::is_project_config(&path) {
            roots.push(dir.to_path_buf());
        }
    }
}

fn lint_corpus(subdir: &str) -> Vec<likec4_lint::Diagnostic> {
...
fn official_examples_have_no_errors() {
    ...
    for rule in rules {
        assert!(likec4_lint::rules().iter().any(|r| r.id == rule), "unregistered rule id {rule}");
    }
}
