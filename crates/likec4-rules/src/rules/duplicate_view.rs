        ...
        for view in &project.views {
            match first.get(view.name.as_str()) {
                ...
                Some(original) => cx.report_related(
                    ...
                    Some("rename or remove one of the views".to_string()),
                    *original,
                    "first declared here",
                ),
            }
...
mod tests {
    ...
    fn reports_duplicates_across_view_kinds() {
        ...
        assert_eq!(diags[0].help.as_deref(), Some("rename or remove one of the views"));
        assert_eq!(
            diags[0].related,
            Some(RelatedLocation {
                file: PathBuf::from("test.c4"),
                range: TextRange::new(15.into(), 20.into()),
