                ...
                for site in rest {
                    cx.report_related(
                        *site,
                        ...
                        Some("remove one of the declarations".to_string()),
                        *first,
                        "first declared here",
                    );
                }
...
#[cfg(test)]
mod tests {
    use crate::rules::testing::{lint_files, lint_one, messages, of_rule};
    ...
    fn reports_duplicates_in_every_namespace() {
        ...
        let first = &of_rule(&diags, "duplicate-spec")[0];
        let related = first.related.as_ref().expect("first declaration");
        assert_eq!(related.file.to_str(), Some("test.c4"));
        assert_eq!(related.message, "first declared here");
        assert!(related.range.start() < first.range.start());
        assert!(!first.help.as_deref().unwrap_or("").contains("test.c4"));
    }
