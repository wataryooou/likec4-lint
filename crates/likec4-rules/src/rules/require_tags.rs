...
pub fn check(cx: &mut Cx<'_, '_>) {
    let ws = cx.ws;
    let kinds = cx.option_strings("kinds").unwrap_or_default();
    let required = cx.option_strings("tags").unwrap_or_default();
...
mod tests {
    ...
    #[test]
    fn wrongly_typed_tags_option_is_reported() {
        let config = enable(
            "require-tags",
            Level::Warning,
            &[
                ("tags", OptionValue::Integer(1)),
                ("kinds", OptionValue::Array(vec![OptionValue::Integer(2)])),
            ],
        );
        let diags = lint_files(&[("t.c4", SRC)], &[], &config);
        assert_eq!(
            messages(&diags, "require-tags"),
            [
                "option 'kinds' of rule 'require-tags' must be a string or a list of strings",
                "option 'tags' of rule 'require-tags' must be a string or a list of strings",
                "require-tags: no `tags` configured",
            ]
        );
    }
}
