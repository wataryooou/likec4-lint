...
| `self-relation` | warning | a relation whose source and target resolve to the same element, comparing both endpoints via the model's scoping rules from the enclosing element (a name resolves to a child of the enclosing element first, then a more distant descendant of it if there is exactly one, then the same two steps for each ancestor in turn, then the root; `this` / `it` and an omitted source are the enclosing element itself); an endpoint that does not resolve in the project (unknown, or ambiguous between two equally distant descendants) is skipped (`unresolved-reference` covers it), and dynamic view steps are exempt (`a -> a` is a valid self-message) |
| `parent-child-relation` | error | a relation between an element and one of its own ancestors or descendants (`a -> a.b`, `a.b -> a`), resolving both endpoints exactly as `self-relation` does; checked in model and deployment relations (`extend` bodies scope like an element body, so they are covered too) and, for dynamic views, only the first arrow of a step chain (measured against `likec4 validate`: a later continuation arrow is not checked, and `a -> a` self-steps are accepted); `a -> a` itself is left to `self-relation` |
