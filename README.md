...
| `parent-child-relation` | error | yes (`Invalid parent-child relationship`) | a relation between an element and one of its own ancestors or descendants (`a -> a.b`, `a.b -> a`), resolved the same way as `self-relation`; checked in model and deployment relations (including `extend` bodies) and the first arrow of a dynamic-view step chain; `a -> a` is `self-relation`'s concern, not this rule's |
